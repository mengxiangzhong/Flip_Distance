...
public class Trangulation extends JPanel{
	...
	public static void main(String[] args) {
		...
		Flip f;
		int index;
		int[] temcount = new int[5];

		Trangulation.inputstyle = 0;
		Flip.output = true;

		if (Trangulation.inputstyle == 0) {
		...
		index = -1;
		System.out.println("原参数值：" + Trangulation.pk);
		m.cmp();
		jf.add(m);
		f = new Flip(m.step_remained);
		DigitDecomposition dd = new DigitDecomposition(Trangulation.pk);
		dd.create();
		if (m.step_remained == 0)f.state = true;
		if (!f.state && m.step_remained <= Trangulation.pk){
			int[] tem;
			Edge[] v1;
			...
			for (int i = 0; i < dd.count; i++) {
				...
				f.recursion(m, tem, 1, v1, 0,temcount);
				if (f.state) {
					index = i;
					break;
				}
		...
		if (f.state) {
			...
			if (index == -1) System.out.println("0");
				...
				for (int j = 0; j <= dd.result[index][0]; j++) {
					System.out.print(dd.result[index][j] + "	");
				}
				System.out.println();
			}
			for (int i = 0; i < 5; i++) {
				System.out.print(f.resultcount[i] + "	");
			}
			System.out.println();
		}
		...
		if (Trangulation.inputstyle == 0 && (Trangulation.pk = Trangulation.pk - 2) >= 0){
			index = -1;
			dd = new DigitDecomposition(Trangulation.pk);
			dd.create();
			starttime = System.currentTimeMillis();
